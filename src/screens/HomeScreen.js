...
export default function HomeScreen({
  ...
  // Habits are no longer their own list — they're entries inside the
  // unified Tasks list (journal) flagged with recurring:true. Deriving
  // them here keeps this screen oblivious to that internal detail.
  const habits = tasks.filter(t => t.recurring);
  ...
  const todaysTasks    = oneTimeTasks.filter(t => t.date === today);
  ...
  const sectionRenderers = {
    ...
    dailyRoutine: habits.length > 0 ? (
      <View>
        <SectionHeader title="Daily Routine" onPress={() => go('journal')} />
