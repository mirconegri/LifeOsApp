...
//
// This screen now serves as "Tasks" in the nav (see src/config/nav.js —
// the file/component name stayed as JournalScreen/journal on purpose, to
// avoid orphaning data already saved under the 'lifeos_journal'
// AsyncStorage key). It replaces the old Study section entirely.
...
export default function JournalScreen({ journal, setJournal }) {
  ...
  // New: tapping a task now opens it here pre-filled, instead of the tap
  // only being able to toggle done/not-done.
  const openEditModal = (entry) => {
    setEditingId(entry.id);
    setFormText(entry.text);
    setFormSubject(entry.subject || '');
    setFormPriority(entry.priority || 'medium');
    setFormDate(entry.date || todayKey());
    setModalVisible(true);
  };
  ...
  // Reordering happens within a single date group — moving a task from
  // "today" above one from "yesterday" wouldn't mean anything, so each
  // group gets its own independent DraggableList. Committing a reorder
  // splices that group's new order back into the flat `journal` array,
  // preserving every other group's entries untouched.
  const handleReorderGroup = (groupDate, reorderedGroupItems) => {
    setJournal(prev => {
      const others = prev.filter(t => (t.date || todayKey()) !== groupDate);
      return [...others, ...reorderedGroupItems];
    });
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {groupedEntries.length === 0 ? (
        <Card><Text style={styles.emptyText}>No tasks yet.</Text></Card>
      ) : (
        groupedEntries.map(group => (
          <View key={group.date} style={styles.dateGroup}>
            ...
            <DraggableList
              ...
              onReorder={(reordered) => handleReorderGroup(group.date, reordered)}
              itemHeight={64}
              renderItem={(t) => (
          ...
          <View style={styles.modal}>
            ...
            <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.modalBtns}>
                ...
                <TouchableOpacity style={styles.confirmBtn} onPress={saveEntry}>
                  <Text style={styles.confirmBtnText}>{editingId ? 'Save Changes' : 'Save'}</Text>
                </TouchableOpacity>
              </View>
...
const styles = StyleSheet.create({
  ...
  reorderHint: { fontSize: 11, color: COLORS.textSub, marginBottom: 8 },
